...
function Question() {
    ...
    return (
            <div style={{margin:"5%",padding:"50px 0px", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column"}}>
                ...
                <div style={{marginTop:"40px"}}>
                {
                    ...
                    :data.state === "B1" ?
                    (
                        <QuestionCheckBox setMessage={(val:string)=>setAlertMessage(val)} />
                    )
                    :data.state === "B2" ?
                    (
                        <QuestionRadio setMessage={(val:string)=>setAlertMessage(val)} setUserAge={(val:string)=>setAge(val)} />
                    )
                    :data.state === "C1" ?
                    (
